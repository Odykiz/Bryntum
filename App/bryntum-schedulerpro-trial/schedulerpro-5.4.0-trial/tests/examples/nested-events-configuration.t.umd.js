StartTest(async t => {
    ...
    // For unknown reasons Siesta scrolls the button partially out of view in Safari, when it should not scroll at all.
    // Works fine manually
    if (!t.browser.safari) {
        t.it('Should add nested event when clicking add icon', async t => {
            const
                parent     = schedulerPro.eventStore.getById(1),
                childCount = parent.children.length;

            await t.click('$event=1 .add-event');

            t.is(parent.children.length, childCount + 1, 'Child added');
            t.is(parent.lastChild.resource, parent.resource, 'Child assigned to same resource as parent');
        });
    }
});
