Class('BryntumGridTest', {
    ...
    methods : {
        async getRemoteGrid(cfg) {
            ...
            return await me.getGrid(cfg);
        },

        async getGrid(cfg) {
            await document.fonts.ready;

            const grid = this.getGridSync({
                ...cfg,
                sanityCheck : false
            });
            ...
            if (grid.isVisible && cfg?.sanityCheck !== false) {
                await this.checkGridSanity(grid);
            }
        ...
        getGridSync(cfg) {
            ...
            const
                generator      = this.global.DataGenerator,
                generateConfig = {
                    count        : cfg.count ?? 25,
                    randomHeight : cfg.randomHeight,
            ...
            if (generator && !cfg.data && !cfg.store) {
                cfg.data = generator.generateData(generateConfig);
            }

            if (generator && cfg.store && !cfg.store.data && !cfg.store.readUrl) {
                cfg.store.data = generator.generateData(generateConfig);
            }
        ...
        async checkGridSanity(grid, next) {
            const
                cols                   = grid.columns,
            ...
            // If caller called us synchronously, then exited the test, the Grid might have been destroyed.
            if (grid.isDestroyed) {
                return;
            }

            // Allow Scheduler to readjust based on font arriving
            grid.onHeightChange();
            ...
            if (!grid.hideHeaders && grid.element.querySelectorAll('.b-grid-header.b-depth-0').length !== columnCount) {
                this.is(grid.element.querySelectorAll('.b-grid-header.b-depth-0').length, columnCount, 'Nbr columns match nbr of elements');
            }
            ...
            for (i = 0; i < shrinkwrapGroupColumns.length; i++) {
                const
                    ...
                    childTotalWidth = column.children.reduce((result, col) => {
                        return (result += col.element?.offsetWidth || 0);
                    }, 0);
            ...
            if (!grid.hideHeaders) {
                for (i = 0; i < rowCount; i++) {
                    ...
                    for (let col = 0; col < columnCount; col++) {
                        ...
                        if (!this.samePx(cellWidth, headerWidth, 1) && !this.samePx(cellWidth, headerWidth)) {
            ...
            // Allow SubGrid scrollbars to lay out
            await this.waitForAnimationFrame();

            // Check the docked scrollers have the correct widths
            // (Vertical scheduler layout has more stages before it's correct)
            if (!grid.isDestroyed && !(grid.isScheduler && grid.isVertical)) {
                grid.eachSubGrid(subGrid => {
                    const
                        { virtualScrollerElement } = subGrid;

                    // Docked fake scroller elements must match their subgrid's width
                    // Check this.samePx to reduce unwanted test output
                    if (virtualScrollerElement &&
                        this.isElementVisible(virtualScrollerElement) &&
                        !this.samePx(subGrid.element.offsetWidth, virtualScrollerElement.offsetWidth)) {
                        this._subTest.isApproxPx(subGrid.element.offsetWidth, virtualScrollerElement.offsetWidth, `SubGrid ${subGrid.id} scroller width matches`);
                    }
                });
            }
        ...
        assertHeaderPosition(doc) {
            const
                contentElBox = doc.querySelector('.b-export-content').getBoundingClientRect(),
        ...
        assertRowsExportedWithoutGaps(doc, assertStartsWithRow = false, assertEndsWithRow = false, assertLastRowVisible = true) {
            const regions = Array.from(doc.querySelectorAll('.b-grid-subgrid'));
            let pass = true;
        ...
        /**
         * Checks that the grid's selected records or cells correspond with provided arguments. Checks selection data and CSS Classes.
         * @param {Grid.view.Grid} grid
         * @param {Number[]} selectedIds Record ids to check. Provide `null` not to check record selection.
         * @param {String} cellSelectors A string representing a cell selector. First row, first cell is 0-0. For multiple selectors, separate with comma (0-0,1-0,2-0)
         * @param {String} [message] Optional message
         */
        assertSelection(grid, selectedIds, cellSelectors, message) {
            const me = this;

            if (selectedIds) {
                const gridSelectedIds = grid.selectedRecords.map(e => e.id);
                me.is(JSON.stringify(gridSelectedIds), JSON.stringify(selectedIds), message || 'Grid\'s selectedRecords correct');
            }
            if (cellSelectors) {
                me.is(grid.selectedCells.map(c => c.rowIndex + '-' + c.columnIndex).join(','), cellSelectors, message || 'Grid\'s selectedCells is correct');
            }

            grid.store.forEach(record => {
                const id  = record.id,
                    row = grid.rowManager.getRowFor(record);

                if (row) {
                    const
                        checkboxCell  = grid.getCell({
                            id,
                            column : grid.checkboxSelectionColumn
                        }),
                        rowIsSelected = selectedIds?.includes(id) || false;

                    if (checkboxCell && checkboxCell.widget.checked !== rowIsSelected) {
                        me.fail(`Cell[${id}, ${grid.checkboxSelectionColumn.data.id}] : checked state is incorrect`);
                    }

                    for (const subGrid in grid.subGrids) {
                        const rowEl = row.elements[subGrid];
                        if (rowEl.classList.contains('b-selected') !== rowIsSelected) {
                            me.fail(`row[${id}] : "b-selected" class state is incorrect`);
                        }

                        row.getCells(subGrid).forEach(cell => {
                            const columnId = this.global.DomDataStore.get(cell, 'columnId');
                            let selected   = false;

                            for (const selectedCell of grid.selectedCells) {
                                if (selectedCell.id === id && selectedCell.columnId === columnId) {
                                    selected = true;
                                    break;
                                }
                            }

                            if (cell.classList.contains('b-selected') !== selected) {
                                me.fail(`cell at row ${id} column ${columnId} "b-selected" class state is correct`);
                            }
                        });
                    }
                }
